...
async function performAction(e) {
    ...
    data = await getWheather(baseURL, zipCode, OpenWeatherApiKey);
    await postData('/add', {date:newDate, location:data.name, temp:data.main.temp, feelings:feelings});
    await updateUI();
}

/* Function to GET Web API Data*/
const getWheather = async (baseURL, zipCode, OpenWeatherApiKey)=>{
    const res = await fetch(baseURL+zipCode+OpenWeatherApiKey);
    try {
